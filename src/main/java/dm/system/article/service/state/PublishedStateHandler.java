...
public class PublishedStateHandler implements ArticleStateHandler {

    private final ArticleService articleService;

    public PublishedStateHandler(ArticleService articleService) {
    ...
    public void transition(ArticleTransitionRequest request, ArticleState targetState) {
        switch (targetState) {
            case UNPUBLISHED, ARCHIVED -> {
                articleService.moveFileInS3(request.getArticleId(), request.getArticleCurrentState(), request.getArticleNewState());
                break;
            }
            default -> throw new IllegalStateException("Invalid transition from PUBLISHED to " + targetState);
        }
    }
}
